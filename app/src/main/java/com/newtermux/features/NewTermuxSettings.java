...
public class NewTermuxSettings {
    ...
    // Internal one-time gate: set once we have shown the all-files-access nudge.
    public static final String KEY_ALL_FILES_PROMPTED        = "all_files_prompted";
    // Internal one-time gate: set once all-files access was granted and storage symlinks were set up.
    public static final String KEY_ALL_FILES_DONE            = "all_files_done";
    // Run manually typed pkg/apt install commands in a throwaway session that closes when done.
    public static final String KEY_PACKAGE_AUTOCLOSE         = "package_autoclose";
    ...
    // One-time all-files-access nudge gate.
    public static boolean wasAllFilesPrompted(Context ctx) {
        return prefs(ctx).getBoolean(KEY_ALL_FILES_PROMPTED, false);
    }
    public static void setAllFilesPrompted(Context ctx, boolean v) {
        prefs(ctx).edit().putBoolean(KEY_ALL_FILES_PROMPTED, v).apply();
    }
    public static boolean wasAllFilesDone(Context ctx) {
        return prefs(ctx).getBoolean(KEY_ALL_FILES_DONE, false);
    }
    public static void setAllFilesDone(Context ctx, boolean v) {
        prefs(ctx).edit().putBoolean(KEY_ALL_FILES_DONE, v).apply();
    }

    // Manually typed pkg/apt install commands run in a throwaway session that auto-closes.
    public static boolean isPackageAutocloseEnabled(Context ctx) {
        return prefs(ctx).getBoolean(KEY_PACKAGE_AUTOCLOSE, true);
    }
    ...
    public static boolean get(Context ctx, String key) {
        switch (key) {
            ...
            case KEY_PACKAGE_AUTOCLOSE:            return isPackageAutocloseEnabled(ctx);
