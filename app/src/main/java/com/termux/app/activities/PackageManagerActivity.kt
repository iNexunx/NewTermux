...
/** Paquete instalado mediante un instalador externo (p. ej. core-termux) en lugar de pkg. */
private const val CORE_TERMUX = "core-termux"
private const val CORE_TERMUX_INSTALL_CMD = "curl -fsSL https://raw.githubusercontent.com/DevCoreXOfficial/core-termux/main/install.sh | bash\n"

/**
 * Navegar entre los paquetes instalados (dpkg) y disponibles (catálogo incluido),
 * con instalación / desinstalación delegada a la terminal. Migración a Compose (Fase 3).
 */
...
private fun PackageManagerScreen(
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Administrador de paquetes") },
                navigationIcon = {
    ...
    detail?.let { entry ->
        ...
        val message = buildString {
            ...
            if (entry.name == CORE_TERMUX) {
                append("Instalador: core-termux (script de instalación oficial)\n\n")
            } else {
                append("Repositorio: Repositorio de paquetes de Termux\npkg.termux.dev\n\n")
            }
        ...
        AlertDialog(
            ...
            confirmButton = {
                if (installedTab) {
                    ...
                    TextButton(onClick = {
                        detail = null
                        val cmd = if (entry.name == CORE_TERMUX) CORE_TERMUX_INSTALL_CMD else "pkg install -y ${entry.name}\n"
                        onRunCommand(cmd)
                    }) { Text(label) }
                }
            },
    ...
    confirmUninstall?.let { entry ->
        AlertDialog(
            ...
            text = { Text("Esto eliminará el paquete de tu entorno Termux.") },
            confirmButton = {
